...
<script>
    ...
    export default {
        ...
        created() {
            //bus.$on('loadScatterData', this.InitializationDataMethod);
            ...
            api.GetTotalCountRes().then(res =>{
                ...
                numdata.forEach(item =>{
                    if(item.type == '小散乱污' || item.type == '散乱污企业'){
                        if(item.area == '广阳区'){
        ...
        methods: {
            ...
            RowCurrentChange(val) {
                this.currentRow = val;
                //地图联动
                bus.$emit('locationClick', 'layer_sp_slw', this.currentRow ,this.type);
            },
            //开发区进度
            KaiFaQuProgressPie() {
            ...
            getPointByType(type, data) {
                ...
                if (dt) {
                    for (let i = 0, length = dt.length; i < length; i++) {
                        let item = dt[i];
                        if (item.TypeName == type || item.TypeName ==='小散乱污') {
                            rtValue.push(dt[i]);
                        }
                    }
                }
